...
- **Ray-plane intersection:** for plane `(p,n)` and ray `z d`, solve `z=(p dot n)/(d dot n)`. Discard nearly parallel rays and negative depths.
...
| Red-excess mask and row-wise weighted centroid | Fast, reduces a thick laser stripe to one sample per row | Fails with red objects/specular highlights; background subtraction or a calibrated laser wavelength filter would improve it. |
